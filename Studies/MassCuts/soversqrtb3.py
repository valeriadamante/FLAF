...
if __name__ == "__main__":
    ...
    df_bckg_filtered = dfWrapped_bckg.df.Filter(filter_base)
    # Loop ottimizzato sulle masse
    masses = [250, 260, 270, 280, 300, 320, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 1000, 1250, 1500, 1750, 2000, 2500, 3000]
    soverb_nocut, soverb_old_linCut, soverb_new_linCut = [], [], []
    ...
    for mass in masses:
        ...
        # Pre-filtraggio
        df_sig_filtered = dfWrapped_sig.df.Filter(filter_base)
        # df_bckg_filtered = df_bckg.Filter(filter_base)

        # Calcola s/√b per ogni condizione
        soverb_nocut.append(df_sig_filtered.Count().GetValue() / math.sqrt(df_bckg_filtered.Count().GetValue()))
        soverb_old_linCut.append(
            df_sig_filtered.Filter(old_linCut).Count().GetValue() /
            math.sqrt(df_bckg_filtered.Filter(old_linCut).Count().GetValue())
        )
        soverb_new_linCut.append(
            df_sig_filtered.Filter(new_linCut).Count().GetValue() /
            math.sqrt(df_bckg_filtered.Filter(new_linCut).Count().GetValue())
        )
    print(masses)
    print(soverb_nocut)
    ...
    # hep.cms.lumitext("Run 2, 13 TeV")
    # plt.savefig("sqrtsbprova.pdf", format="pdf", bbox_inches="tight")
    # plt.savefig("sqrtsbprova.png", bbox_inches="tight")

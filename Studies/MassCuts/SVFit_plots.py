...
def getLabels(cat,year,channel):
    labels = ["$m_{\\tau\\tau}^{SVFit}$","$m^{vis}_{\\tau\\tau}$"]
    ...
    return labels,cat_name,channelname,title,outFileName
...
def PlotMass(df, hist_cfg_dict, global_cfg_dict,filter_str,cat,channel='tauTau', year='2018', return_hists=False):
    ...
    labels,cat_name,channelname,title,outFileName = getLabels(cat,"all",channel)
    hist_list = []
...
if __name__ == "__main__":
    ...
    for year in years_list:
        ...
        for channel in channels:
            ...
            for cat in cats:
                ...
                hists[(channel,cat)][year]["sig"] = PlotMass(dfWrapped_sig.df,hist_cfg_dict,global_cfg_dict, filter_str,cat,channel, args.year, return_hists=True)
                hists[(channel,cat)][year]["DY"] = PlotMass(dfWrapped_bckg.df,hist_cfg_dict,global_cfg_dict, filter_str,cat,channel, args.year, return_hists=True)
                # hists[(channel,cat)][year]["Htt"] = PlotMass(dfWrapped_bckg.df,hist_cfg_dict,global_cfg_dict, filter_str,cat,channel, args.year, return_hists=True)
    bins = hist_cfg_dict["SVfit_m"]["x_rebin"][cat] if cat in hist_cfg_dict["SVfit_m"]["x_rebin"].keys() else hist_cfg_dict["SVfit_m"]["x_rebin"]["other"]
    # print(hists)
    ...
    for channel in channels:
        ...
        for cat in cats:
            print(cat)
            hist_cat_list_sig = hists[(channel,cat)][years_list[0]]["sig"]
            hist_cat_list_DY = hists[(channel,cat)][years_list[0]]["DY"]
            for year_idx in (1, len(years_list)-1):
                year_value = years_list[year_idx]
                hist_cat_list_sig_2 = hists[(channel,cat)][year_value]["sig"]
                hist_cat_list_DY_2 = hists[(channel,cat)][year_value]["DY"]
                for hist1_sig,hist2_sig in zip(hist_cat_list_sig,hist_cat_list_sig_2):
                    hist1_sig.Add(hist2_sig)
                for hist1_DY,hist2_DY in zip(hist_cat_list_DY,hist_cat_list_DY_2):
                    hist1_DY.Add(hist2_DY)
                # for hist1_DY,hist2_DY in zip(hist_cat_list_DY,hist_cat_list_DY_2):
                #     hist1_DY.Add(hist2_DY)
            labels,cat_name,channelname,title,outFileName=getLabels(cat,"all",channel)

            plot_1D_histogram_SV(hist_cat_list_sig, labels, f"{title} $HH\\rightarrow bb \\tau\\tau$",  f"{outFileName}_sig", "Run2_all")
            plot_1D_histogram_SV(hist_cat_list_DY, labels, f"{title} DY", f"{outFileName}_DY", "Run2_all")

            print(outFileName+"_sig.pdf")
            print(outFileName+"_DY.pdf")

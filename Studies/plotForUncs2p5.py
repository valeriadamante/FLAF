...
def getInitialHisto(dir_1, processes,unc_name='', n_initial=0):
    full_name = processes[n_initial]
    if unc_name !='':
        full_name = f"{processes[n_initial]}_{unc_name}"

    hist_initial = dir_1.Get(full_name)
    if hist_initial.GetEntries() !=0 :
        return hist_initial, n_initial
    while hist_initial.GetEntries() ==0:
        n_initial +=1
        full_name = processes[n_initial]
        if unc_name !='':
            full_name = f"{processes[n_initial]}_{unc_name}"
        hist_initial = dir_1.Get(full_name)
    hist_initial.SetDirectory(0)
    return hist_initial, n_initial
...
if __name__ == "__main__":
    ...
    hist_central, n_initial = getInitialHisto(dir_1, processes)

    should_continue=False
    for process in processes[n_initial+1:]:
        hist_to_add_central = dir_1.Get(process)
        if hist_to_add_central.GetEntries() ==0 :
            continue
    ...
    for unc in unc_names:
        ...
        if f"{processes[0]}_{full_name_unc_up}" not in keys_cat:
            print(f"{processes[0]}_{full_name_unc_up} not present")
            should_continue = True
        if f"{processes[0]}_{full_name_unc_down}" not in keys_cat:
            print(f"{processes[0]}_{full_name_unc_down} not present")
            should_continue = True
        if should_continue : continue
        hist_up, n_initial_up = getInitialHisto(dir_1, processes, full_name_unc_up)
        hist_down, n_initial_down = getInitialHisto(dir_1, processes, full_name_unc_down)
        ...
        n_initial_unc = n_initial_up if n_initial_up < n_initial_down else n_initial_down
        # print(f"initially hist up has {hist_up.GetEntries()} entries")
        # print(f"initially hist down has {hist_up.GetEntries()} entries")
        for process in processes[n_initial_unc:]:
            # print(f"process {process}")
            hist_to_add_up = dir_1.Get(f"{process}_{full_name_unc_up}")
            hist_to_add_up.SetDirectory(0)
            # print(f"hist up has {hist_to_add_up.GetEntries()} entries")
            if hist_to_add_up.GetEntries() ==0 :
                continue
            hist_up.Add(hist_to_add_up)
            hist_to_add_down = dir_1.Get(f"{process}_{full_name_unc_down}")
            hist_to_add_down.SetDirectory(0)
            # print(f"hist up has {hist_to_add_down.GetEntries()} entries")
            if hist_to_add_down.GetEntries() ==0 :
                continue
        ...
        plt.savefig(f"{outDir}/{full_name_unc_up}.pdf", format="pdf", bbox_inches="tight")
        plt.savefig(f"{outDir}/{full_name_unc_up}.png", format="png", bbox_inches="tight")
        print(f"{outDir}/{full_name_unc_up}.png")